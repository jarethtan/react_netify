...
function App() {
  ...
  const [newItem, setNewItem] =
    useState(
      ""
    ); /* initial state should be blank since there should not be any default item to be added. The user has to add it in */
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        setFetchError(
          null
        ); /* we want to reset the setFetchError to null in case there is a fetchError set which has an error in there. */
      } catch (err) {
  ...
  const addItem = async (item) => {
    const id = items.length
      ...
      : 1; /* This line tells us that if there items in the list (items.length), from the id of the last item in the list ( items[items.length -1].id ) and + 1. If there is no item on the list, the id will be set to 1 ( : 1 )  */
    const myNewItem = {
      id,
      checked: false,
      item,
    }; /* this is only 'item' because the object which got pass through to this function is the description of the item. in the input field on the AddItem.js file, it stores the value which is the description of the item. */
  ...
  const handleCheck = async (id) => {
    ...
    ); // "checked: !item.checked" means flipping the check status. if it is false, it will go true. if it is true, it will go false. rem we spread the item in the first place because only that way we can get it out of an array.
    setItems(listItems);
  ...
  const handleSubmit = (e) => {
    ...
    if (!newItem)
      return; /* if there is no input in the list, it should not submit anything */
